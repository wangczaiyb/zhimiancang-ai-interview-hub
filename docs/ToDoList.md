...
| 优先级 | 模块 | 报告做法 | 本项目现状 | 为什么值得补|
| :--- | :--- | :--- | :---: | :--- |
| P0 | 结构化题库 + 组卷引擎 | 6 岗位 × 60 道预置题，按专业:通用:压力 = 5:3:2 抽题，去重打乱 | 完全没有题库表，所有题实时 AI 生成（`source` 恒为`AI_GENERATED` ） | 三重收益：① 无网/无 Key/额度耗尽时功能 100% 可用；② AI 成本从"每题一次调用"降到"仅评分"；③ 同岗位题目稳定，学生成绩可横向比较 |
| P0 | 卷面 JSON 固化（历史可复现） | 组卷结果以 JSON 快照写入会话记录 | 题目逐条落库但无快照，题库若被编辑则历史失真 | 与已有`jd_text` 快照思路一致，顺手可做 |
| P1 | 参考答案要点 + 逐题对照 | 每题附「参考答案要点」+ 逐题诊断，学生可对照补短板、教师可据此教学 | 只有 Rubric 证据/弱点/建议， 没有参考答案 ，学生不知道"标准答案长什么样" | 这是"测—评—学"闭环的关键缺口，直接接现有学习路线 |
| P1 | 会话状态机 + 非法迁移拦截 | `created→in_progress→evaluating→finished→archived` 集中声明，非法迁移入口即返 409；支持 abort | 无状态机，`start/finish` 直接赋值 status； 重复提交同题会静默覆盖重评，结算后仍可提交 ；`CANCELLED` 只存在于注释，没有任何接口能中止面试 | 属于 正确性缺陷修复 ，不只是新功能 |
| P1 | 逐题倒计时 + 超时处理 + 字数统计 | 倒计时环、进度条、字数统计、分类临场提示；超时不归零只轻扣 | 只有整场 30 分钟倒计时， 归零不自动提交 ；无逐题限时、无字数统计 | 报告明确"超时提交不报错且给出反馈"，这是压力面试还原的核心 |
| P2 | 岗位候选人匹配度排行榜 | 按匹配度排序的榜单 + 成绩段分布 | `/enterprise/candidates` 已算出`match_score` 但按`Application.id desc` 排序，等于没排序 | 数据已就位，只差排序 + 榜单视图，性价比极高 |
| P2 | 评分通道可观测接口 | `GET /api/channel` 返回当前通道与降级原因；LLM 异常自动降级不扩散 | 有 mock 回退但 不可观测 ；`AICallLog` 表定义了却 全库零写入点、零查询端点 | 正好合并进待办第 3 批"管理端 AI 统计真实化" |
| P2 | 题型三分类（专业/通用/压力） | 三类题、压力题专门给情绪应对反馈 | `mode` 里有`STRESS` 且前端可选，但后端只存字段，组卷配比写死为"专业基础2/深度探究1/项目深挖1/系统设计1"， 不随 mode 变化 | 选压力面试却拿不到压力题，是明显的功能空转 |
| P3（可选） | 零依赖单进程演示模式 `python main.py` 一条命令、无构建无 CDN | 需 venv + pip + npm install + Vite | 竞赛/评审现场可能断网，值得留一个"打包演示"开关（如后端直接托管`dist/` ）| 后端直接托管`dist/` ）|

## 二、需要修改的本项目自身问题（报告反衬出来的）
这几处 报告是诚实的、本项目名不副实 ，建议优先处理：

1. README 与代码不符 ：README 写"系统自动从题库或 AI 生成"，实际无题库。→ 要么补题库，要么改文案。
2. `duration_sec` 是假数据 ：前端写死上报 50，服务端不校验。而 Rubric 六维里有"作答节奏"，等于 用一个假字段在给分 。→ 服务端按`started_at` 真实计算，或从评分维度里剔除。
3. `speaking_rate` /`filler_count` /`audio_file_id` 是空壳字段 ：默认值 160/2 兜底，无语音上传与分析链路。若报告页展示会直接误导用户。→ 要么删除，要么按报告"后续演进"方向接 ASF/ASR。
4. 成长曲线与报告有硬编码兜底 ：`/personal/growth` 无数据时返回写死的 68/74/82；`interviews.py` 报告兜底写死五维分。→ 无数据应返回空态而非假趋势。
另外，报告暴露的 统计真实化缺口 与你`docs/功能完善与实现建议.md` 第 3 批完全重合：企业端`get_analytics` （`interview_rate: 65` 写死）、管理端`get_ai_providers` （`success_rate: 99.8` 写死）。报告里的「7 项核心指标 + 岗位训练量柱状图 + 成绩段分布」可以直接作为这两个接口的目标形态。
